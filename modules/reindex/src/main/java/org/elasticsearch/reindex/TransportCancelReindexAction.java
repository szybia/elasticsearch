...
public class TransportCancelReindexAction extends TransportTasksAction<
    ...
    @Inject
    public TransportCancelReindexAction(final ClusterService clusterService,
    ...
    protected CancelReindexResponse newResponse(
        ...
        assert tasks.size() <= 1 : "currently only supports cancelling one task max";
        final Supplier<ResourceNotFoundException> notFoundSupplier = () -> new ResourceNotFoundException("reindex task [{}] either not found or completed", request.getTargetTaskId());
        for (FailedNodeException e : failedNodeExceptions) {
            if (ExceptionsHelper.unwrap(e, NoSuchNodeException.class) != null) {
                throw notFoundSupplier.get();
            }
        }

        final var response = new CancelReindexResponse(taskOperationFailures, failedNodeExceptions);
