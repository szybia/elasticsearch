...
import org.elasticsearch.reindex.CancelReindexRequest;
import org.elasticsearch.reindex.CancelReindexResponse;
import org.elasticsearch.reindex.ReindexTestCase;
import org.elasticsearch.reindex.TransportCancelReindexAction;
...
public class ReindexCancelIT extends ReindexTestCase {
    ...
    public void testCancelEndpointEndToEndSynchronously() throws Exception {
        ...
        assertThat(cancelSubTaskException.getMessage(), is(Strings.format("reindex task [%s] either not found or completed", firstSubTask)));
        ...
        for (final TaskId taskId : List.of(parentTaskId, firstSubTask)) {
            final RawTaskStatus taskStatus = (RawTaskStatus) getCompletedTaskResult(taskId).getTask().status();
            final String cancelledReason = (String) taskStatus.toMap().get("canceled");
    ...
    public void testCancelEndpointEndToEndAsynchronously() throws Exception {
        ...
        final TaskId firstSubTask = parent.childTasks().getFirst().task().taskId();
        final var cancellingSubTaskException = expectThrows(ResourceNotFoundException.class, () -> cancelReindexSynchronously(firstSubTask));
        assertThat(cancellingSubTaskException.getMessage(), is(Strings.format("reindex task [%s] either not found or completed", firstSubTask)));
